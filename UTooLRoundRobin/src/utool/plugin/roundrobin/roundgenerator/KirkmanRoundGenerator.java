...
/**
 * This round generator is responsible for creating a round using
 * the kirkman tournament algorithm
 * 
 ...
 * @version 3/23/2013
 */
public class KirkmanRoundGenerator extends RoundGenerator{
	...
	public Round generateRound(List<RoundRobinPlayer> orderedPlayers, RoundRobinTournament t) 
	{
		...
		{
			Log.e("KirkmanRoundGEnerator","Error: Orderedplayers or passed in tournaemnt was null");
			return null;
		}

		int round = t.getRounds().size();
		Log.d("asid","Round: "+round);
		...
		if(round==0)
		{
			...
			//create the rounds
			//TODO for(int i=0;i<indicies.size();i++)
			{
				//create the rounds
			...
			t.setGraph(graph);
			int index=graph.get(round).size()-1;
			//create the list of matches

			for(int m=0;m<graph.get(round).size()/2;m++)
			{
				RoundRobinPlayer p1 =null;
				RoundRobinPlayer p2 =null;

				if(graph.get(round).get(m).equals(Player.BYE))
				{
				...
				else
				{
					for(int i = 0;i<orderedPlayers.size();i++)
					{
						if(orderedPlayers.get(i).getUUID().equals(graph.get(round).get(m)))
						{
				...
				if(graph.get(round).get(index).equals(Player.BYE))
				{
				...
				else
				{
					for(int i = 0;i<orderedPlayers.size();i++)
					{
						if(orderedPlayers.get(i).getUUID().equals(graph.get(round).get(index)))
						{
